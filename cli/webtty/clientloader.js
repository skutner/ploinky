...
(function(){
  ...
  let term, fitAddon;
  function initConsole() { const termEl = document.getElementById('term'); const { Terminal } = window; const FitAddon = window.FitAddon.FitAddon; const WebLinksAddon = window.WebLinksAddon.WebLinksAddon; const termBg = getComputedStyle(document.body).getPropertyValue('--term-bg').trim() || '#111b21'; term = new Terminal({ fontFamily: 'Menlo, Monaco, Consolas, monospace', fontSize: 13, theme: { background: termBg }, cursorBlink: true, cursorStyle: 'bar', allowProposedApi: true, convertEol: true, scrollback: 2000, rendererType: 'canvas' }); fitAddon = new FitAddon(); const links = new WebLinksAddon(); term.loadAddon(fitAddon); term.loadAddon(links); term.open(termEl); term.focus(); try { fitAddon.fit(); } catch (e) { dlog('fit error', e); } sizeEl.textContent = term.rows + ' x ' + term.cols; termEl.addEventListener('mousedown', () => term.focus()); dlog('console initialized'); }
  let es; let chatBuffer = ''; function stripAnsi(s){ try { return s.replace(/[\u001b\u009b][[()#;?]*(?:[0-9]{1,4}(?:;[0-9]{0,4})*)?[0-9A-ORZcf-nqry=><]/g, ''); } catch(_) { return s; } }
  function pushSrvFromBuffer(){ if (!chatBuffer) return; const parts = chatBuffer.split(/\r?\n/); chatBuffer = parts.pop() || ''; const blocks = parts.filter(Boolean).join('\n'); if (blocks) addServerMsg(blocks); }
  function startSSE(){ dlog('SSE connecting'); showBanner('Connecting…'); try { es?.close?.(); } catch(_){} es = new EventSource('/stream'); es.onopen = () => { statusEl.textContent = 'connected'; dlog('SSE open'); showBanner('Connected', 'ok'); setTimeout(hideBanner, 1000); }; es.onerror = (e) => { statusEl.textContent = 'reconnecting...'; dlog('SSE error', e); showBanner('Reconnecting…'); }; es.onmessage = (ev) => { try { const text = JSON.parse(ev.data); dlog('SSE message', { bytes: (text || '').length }); term.write(text); chatBuffer += stripAnsi(text); pushSrvFromBuffer(); } catch (e) { dlog('term write error', e); } }; es.addEventListener('meta', (ev) => { try { const meta = JSON.parse(ev.data || '{}'); if (typeof meta.clients === 'number') { statusEl.textContent = `connected (${meta.clients})`; dlog('meta', meta); } } catch (e) { dlog('meta parse error', e); } }); }
  let userInputBuf = ''; function bindConsoleIO(){ window.addEventListener('resize', sendResize); setTimeout(sendResize, 120); term.onData(data => { dlog('send input', { bytes: (data||'').length }); fetch('/input', { method: 'POST', headers: { 'Content-Type': 'text/plain' }, body: data }).catch((e)=>{ dlog('input error', e); showBanner('Input error', 'err'); }); try { const s = data || ''; for (let i=0;i<s.length;i++){ const ch = s[i]; if (ch==='\n'||ch==='\r'){ const msg = userInputBuf.trim(); if (msg) addMsg(msg, 'me'); userInputBuf=''; } else { userInputBuf+=ch; } } } catch(_){} }); }
  function sendResize(){ try { fitAddon.fit(); } catch (e) { dlog('fit error', e); } const cols = term.cols; const rows = term.rows; sizeEl.textContent = rows + ' x ' + cols; dlog('send resize', { cols, rows }); fetch('/resize', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ cols, rows }) }).catch((e)=>{ dlog('resize error', e); showBanner('Resize error', 'err'); }); }
  ...
  function autoresize(){ cmdInput.style.height = 'auto'; cmdInput.style.height = Math.min(200, Math.max(44, cmdInput.scrollHeight)) + 'px'; } cmdInput.addEventListener('input', autoresize); setTimeout(autoresize, 0);
